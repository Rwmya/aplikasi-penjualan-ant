...
export async function POST(req: Request) {
  try {
    ...
    const capitalizedItems = data.map(
      (item: { name: string; satuan: string; harga: string }) => ({
        ...
        // createdAt: new Date(),            // Optionally set createdAt here
      }),
    );
    ...
    return NextResponse.json(
      { success: false, error: "Failed to save items" },
      { status: 500 },
    );
  }
}
